...
export function loadConfig(): AppConfig {
  try {
    ...
    const merged: AppConfig = {
      ...
      // Preserve supported saved themes and recover malformed legacy values.
      theme: resolveAppTheme(parsed.theme),
      pet: normalizePet(parsed.pet),
      notifications: normalizeNotifications(parsed.notifications),
      orbit: normalizeOrbit(parsed.orbit),
    };
    // Persist only when an unsupported legacy value had to be normalized.
    let migratedConfig = parsed.theme != null && parsed.theme !== resolveAppTheme(parsed.theme);
