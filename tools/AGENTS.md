...
- `tools/pack` provides `@open-design/tools-pack` and the `tools-pack` bin. The active slice is packaged artifact build/install/start/stop/logs/uninstall/cleanup/list/reset, shell-neutral Standalone Closure archive construction, and release artifact preparation for mac and Windows lanes. Linux has no packaged-delivery promise; source builds remain an expert-owned path.
...
- Keep `tools-pack` focused on packaging/runtime control, release artifact preparation, and the packaged-updater acceptance harness. The updater product surface and launcher handoff live in `shells/electron`; do not duplicate that application logic in the tool.
...
- Orchestration layers must consume primitives from `@open-design/sidecar` and `@open-design/platform`.
