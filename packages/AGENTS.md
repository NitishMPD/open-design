...
- `packages/closure-proto`: pure TypeScript Standalone release protocol. It owns namespace/target-neutral release identity, canonical distribution digest, content-addressed blob mapping, required `launcher/native/body` components, lazy resource locks, artifact integrity/inventory/signature, and shell compatibility metadata. Node is a Shell bootstrap dependency, not a Closure component. Resolving a target must keep lazy resource blobs out of the required cold-start set; the package must not become a second live handoff protocol.
